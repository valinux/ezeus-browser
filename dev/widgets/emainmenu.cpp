...
void eMainMenu::initialize(const eAction& newGameA,
    ...
    // The backdrop now covers the window; anchor this control to the viewport
    // instead of the original image, whose edges may be cropped on resize.
    leader->align(eAlignment::left | eAlignment::top);
    leader->move(2*p, 2*p);
#else
    int tw;
    int th;
