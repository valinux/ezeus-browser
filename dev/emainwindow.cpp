...
#include "widgets/ebrowserlayout.h"

// A hidden single-player city is suspended, including rendering and audio.
// Resume on the visibility event instead of polling or accumulating missed ticks.
EM_ASYNC_JS(void, waitForVisibleGame, (), {
    if (!document.hidden) return;
    var stats = Module['gamePerformance'];
    if (stats) stats['background'] = true;
    var audio = Module['SDL2'] && Module['SDL2'].audioContext;
    var resumeAudio = audio && audio.state === 'running';
    if (resumeAudio) audio.suspend().catch(function() {});
    ...
    if (resumeAudio && audio.state === 'suspended') audio.resume().catch(function() {});
    if (stats) stats['background'] = false;
});

// Idle screens refresh slowly, but input wakes them immediately.
EM_ASYNC_JS(void, waitForGameInput, (int milliseconds), {
    await new Promise(function(resolve) {
        var events = ['pointermove', 'pointerdown', 'pointerup', 'wheel', 'keydown', 'keyup', 'visibilitychange', 'ezeusviewportchange'];
        function wake() {
            clearTimeout(timer);
            events.forEach(function(type) { document.removeEventListener(type, wake, true); });
            resolve();
        }
        var timer = setTimeout(wake, milliseconds);
        events.forEach(function(type) { document.addEventListener(type, wake, {capture: true, passive: true}); });
    });
});
#endif
...
#ifdef __EMSCRIPTEN__
    auto lastInput = steady_clock::now();
#endif
    while(!mQuit) {
#ifdef __EMSCRIPTEN__
        if(EM_ASM_INT({ return document.hidden; })) {
            waitForVisibleGame();
            lastInput = steady_clock::now();
        }
        // JavaScript coalesces resize events. Apply only the latest viewport at
        // a frame boundary, where the engine is not painting or handling input.
        const int browserWidth = EM_ASM_INT({ return Module['browserViewport']?.width || 0; });
        const int browserHeight = EM_ASM_INT({ return Module['browserViewport']?.height || 0; });
        if(browserWidth > 0 && browserHeight > 0 &&
           (browserWidth != width() || browserHeight != height())) {
            setResolution(eResolution(browserWidth, browserHeight));
            if(mGW) {
                eBrowserLayout::resize(mGW, browserWidth, browserHeight);
                mGW->updateViewBoxSize();
            }
            if(mWW) eBrowserLayout::resize(mWW, browserWidth, browserHeight);
            if(mWidget && mWidget != mGW && mWidget != mWW) {
                eBrowserLayout::resize(mWidget, browserWidth, browserHeight);
            }
            lastInput = steady_clock::now();
        }
#endif
        ...
        while(SDL_PollEvent(&e)) {
            ...
            if(e.type == SDL_MOUSEMOTION || e.type == SDL_MOUSEBUTTONDOWN ||
               ...
               e.type == SDL_KEYDOWN || e.type == SDL_KEYUP || e.type == SDL_WINDOWEVENT) {
                lastInput = steady_clock::now();
            }
#endif
            int x, y;
        ...
        eMusic::incTime();
#ifdef __EMSCRIPTEN__
        // Keep the browser's loading bar until an actual menu has been painted,
        // including the frame where the menu loader replaces itself.
        const bool browserMenuLoading = dynamic_cast<eMenuLoadingWidget*>(mWidget);
#endif
        if(mWidget) {
            mWidget->paint(p);
            tooltip.update();
...
#ifdef __EMSCRIPTEN__
        const bool cityPaused = mWidget == mGW && mGW && mGW->settings().fPaused;
        // Loading screens and a running simulation retain the original 20 Hz.
        const bool idleScreen = cityPaused || dynamic_cast<eMainMenu*>(mWidget);
        const bool reduceFrameRate = idleScreen && steady_clock::now() - lastInput > 250ms;
        EM_ASM({
            var stats = Module['gamePerformance'] || (Module['gamePerformance'] = {frames: 0, background: false});
            stats['frames']++;
            stats['paused'] = !!$0;
            stats['targetFps'] = $1;
            stats['boardFrame'] = $2;
            ...
            stats['width'] = $4;
            stats['height'] = $5;
        }, cityPaused, reduceFrameRate ? 2 : 20, mBoard ? mBoard->frame() : 0,
           mBoard ? mBoard->date().day() : 0, width(), height());
        static bool browserFrameReported = false;
        if(!browserFrameReported && !browserMenuLoading) {
            browserFrameReported = true;
            MAIN_THREAD_EM_ASM({
                if(Module['onGameFrame']) Module['onGameFrame']();
            });
        }
#endif
...
#ifdef __EMSCRIPTEN__
        const duration<double, std::milli> fpsDuration(1000./(reduceFrameRate ? 2 : fpsClamp));
#else
        const duration<double, std::milli> fpsDuration(1000./fpsClamp);
#endif
        const duration<double, std::milli> fpsSleep(fpsDuration - fpsElapsed);
#ifdef __EMSCRIPTEN__
        // Yield for input, paint, and audio even when this frame exceeded its budget.
        const int delay = std::max(0, static_cast<int>(fpsSleep.count()));
        if(reduceFrameRate) waitForGameInput(delay);
        else emscripten_sleep(delay);
#else
